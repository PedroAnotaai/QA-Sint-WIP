...
import { apiConfig, loadPayload } from '../utils/apiConfig.js';
...
test.describe('Qualification Rule Service - lead endpoint', () => {
    ...
    qase(81,
        test('[SQL] Social || Tiktok - Pedidos, computador e fit', async ({ request }) => {
            const payload = loadPayload('playwright/fixtures/payload-sql.json')
            payload.email = `${Math.random().toString(36).substring(2, 10)}@anota.ai`;
            payload.utmSource = "Tiktok";
            payload.utmCampaign = "-whatsapp";

            const response = await request.post(`${apiConfig.baseURL}/lead`, {
                headers: {
            ...
            expect(response.body).to.have.property('tierQualification', '');
